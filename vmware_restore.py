...
class VMwareRestore:
    ...
    def restore_vm(self, backup_path: str, new_vm_name: str = None, 
        ...
        try:
            ...
            vm_config = self._create_vm_config(vm_info, uploaded_files, datastore, vm_name)
    ...
    def _create_vm_config(self, vm_info: Dict, vmdk_files: List[str], 
                         datastore: vim.Datastore, vm_name: str) -> vim.vm.ConfigSpec:
        ...
        # Verwende bevorzugt den aktuellen VM-Namen (kann vom Original abweichen)
        config_spec.name = vm_name or vm_info.get('name', 'Restored_VM')
        ...
        # Setze vmPathName, damit ESXi weiß, wo die .vmx liegt
        vm_folder = vm_name.replace(' ', '_')
        files = vim.vm.FileInfo()
        files.vmPathName = f"[{datastore.name}] {vm_folder}/{vm_folder}.vmx"
    ...
    def _get_resource_pool(self) -> Optional[vim.ResourcePool]:
        """Gibt einen ResourcePool für die VM-Erstellung zurück"""
        if not self.content:
            return None
    ...
    def _register_vm(self, config_spec: vim.vm.ConfigSpec, vm_name: str,
        ...
        try:
            ...
            if not resource_pool:
                if progress_callback:
                    progress_callback("Fehler: Kein ResourcePool gefunden")
                return None
